...
def cmass_poly_mask(cmass, mask, swapxy):
...
if in_mask is None:
    ...
    # Distance transform...
    mask = sp.ndimage.morphology.distance_transform_edt(np.invert(mask)) * rf_res
    if mk_rsc != 1:
        mask = sp.ndimage.interpolation.zoom(mask, mk_rsc)
    mask = mask <= mk_dst
print '\t\t-Number of particles in the tomogram: ' + str(len(cmass))
cmass_poly = cmass_poly_mask(cmass, mask, rt_swapxy)
if rt_swapxy:
    mask = np.swapaxes(mask, 0, 1)
